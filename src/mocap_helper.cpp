...
int main(int argc, char **argv)
{
  ...
  ros::Publisher goal_pub = nl.advertise<geometry_msgs::Pose>("/goal_pos", 1000);
  ros::Rate loop_rate(20);

  std::map<std::string, std::string> cfg;
  std::string hostname = "169.254.217.218";
  ...
  for (size_t frameId = 0; ros::ok(); ++frameId) {
    ...
    try{
        ...
        for (auto const& kv: rigidBodies) {
          ...
          goal_msg.position.x = position(0);
          goal_msg.position.y = position(1);
          goal_msg.position.z = position(2);

          goal_msg.orientation.x = rotation.vec()(0);
          goal_msg.orientation.y = rotation.vec()(1);
          goal_msg.orientation.z = rotation.vec()(2);
          goal_msg.orientation.w = rotation.w();
